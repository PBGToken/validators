...
import contract, { GOV_KEY_1, GOV_KEY_2, GOV_KEY_3 } from "pbg-token-validators-test-context"
...
describe("governance_delegate::main", () => {
    ...
    const configureContext = (props?: {govKey2?: PubKeyHash, govKey3?: PubKeyHash}) => {
    ...
    it("succeeds if signed by majority", () => {
        configureContext({govKey3: PubKeyHash.dummy()})
            .use(ctx => {
                main.eval({$scriptContext: ctx, _: redeemer})
            })
    })
    ...
    it("throws an error if only signed by one key", () => {
        configureContext({govKey2: PubKeyHash.dummy(), govKey3: PubKeyHash.dummy()})
            .use(ctx => {
                throws(() => {
                    main.eval({$scriptContext: ctx, _: redeemer})
                })
