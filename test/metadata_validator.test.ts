...
import { ConfigChangeProposal, ConfigType, castMetadata, makeConfig, makeMetadata } from "./data";
...
describe("metadata_validator::main", () => {
    ...
    const configureContext = (props?: {config?: ConfigType, hash?: number[], proposal?: ConfigChangeProposal}) => {
    ...
    it("throws an error if the hash doesn't correspond to the new metadata", () => {
        configureContext({hash: dummyBytes(32, 0)})
            .use(ctx => {
                throws(() => {
                    main.eval({$scriptContext: ctx, $datum: metadata, _: redeemer})
                })
    ...
    it("throws an error if not ChangingMetadata state", () => {
        configureContext({proposal: {ChangingAgent: {agent: new PubKeyHash(actualHash.slice(0, 28))}}})
            .use(ctx => {
                throws(() => {
                    main.eval({$scriptContext: ctx, $datum: metadata, _: redeemer})
                })
    ...
    it("throws an error if in Idle state", () => {
        configureContext({config: makeConfig()})
            .use(ctx => {
                throws(() => {
                    main.eval({$scriptContext: ctx, $datum: metadata, _: redeemer})
                })
