...
import contract, { ORACLE_KEY_1, ORACLE_KEY_2, ORACLE_KEY_3 } from "pbg-token-validators-test-context"
...
describe("oracle_delegate::main", () => {
    ...
    const configureContext = (props?: {oracleKey2?: PubKeyHash, oracleKey3?: PubKeyHash}) => {
    ...
    it("succeeds if signed by majority", () => {
        configureContext({oracleKey3: PubKeyHash.dummy()})
            .use(ctx => {
                main.eval({$scriptContext: ctx, _: redeemer})
            })
    })
    ...
    it("throws an error if only signed by one key", () => {
        configureContext({oracleKey2: PubKeyHash.dummy(), oracleKey3: PubKeyHash.dummy()})
            .use(ctx => {
                throws(() => {
                    main.eval({$scriptContext: ctx, _: redeemer})
                })
